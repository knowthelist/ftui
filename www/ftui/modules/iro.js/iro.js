...
function () {
  ...
  */
  ...
  const _proto = IroColor.prototype;
  ...
  */
  ...
  _createClass(IroColor, [{
    ...
    key: 'rgb',
    get: function get() {
      const _IroColor$hsvToRgb = IroColor.hsvToRgb(this.$),
    ...
    key: 'hsl',
    get: function get() {
      const _IroColor$hsvToHsl = IroColor.hsvToHsl(this.$),
...
function getSliderValueFromInput(props, x, y) {
  const _getSliderDimensions = getSliderDimensions(props),
...
function getSliderHandlePosition(props, color) {
  const _getSliderDimensions2 = getSliderDimensions(props),
  ...
  switch (props.sliderType) {
    ...
    case 'kelvin':
      ...
      for (let kelvin = min, stop = 0; kelvin < max; kelvin += range / numStops, stop += 1) {
        const _IroColor$kelvinToRgb = IroColor.kelvinToRgb(kelvin),
...
const mod = function mod(a, n) {
  return (a % n + n) % n;
}; // distance between points (x, y) and (0, 0)


const dist = function dist(x, y) {
...
function isInputInsideWheel(props, x, y) {
  const _getWheelDimensions = getWheelDimensions(props),
...
function getWheelHandlePosition(props, color) {
  ...
  const _getWheelDimensions2 = getWheelDimensions(props),
...
function getWheelValueFromInput(props, x, y) {
  const _getWheelDimensions3 = getWheelDimensions(props),
...
function getBoxValueFromInput(props, x, y) {
  const _getBoxDimensions = getBoxDimensions(props),
...
function getBoxHandlePosition(props, color) {
  const _getBoxDimensions2 = getBoxDimensions(props),
...
const SECONDARY_EVENTS = ['mousemove' /* MouseMove */, 'touchmove' /* TouchMove */, 'mouseup' /* MouseUp */, 'touchend' /* TouchEnd */];
// Base component class for iro UI components
// This extends the Preact component class to allow them to react to mouse/touch input events by themselves
const IroComponentWrapper = /*@__PURE__*/(function (Component) {
...
function IroBox(props) {
  ...
  const handlePositions = colors.map(function (color) { return getBoxHandlePosition(props, color); });
...
function IroWheel(props) {
  ...
  const handlePositions = colors.map(function (color) { return getWheelHandlePosition(props, color); });
  const circleStyles = {
    position: 'absolute',
    top: 0,
    left: 0,
